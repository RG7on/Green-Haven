  ...
  return (
    <div className="card" style={{padding:'1rem', borderRadius:'16px'}}>
      ...
      <div className="stack" style={{padding:'0.75rem'}}>
        <div style={{minHeight:56}}>{product.name}</div>
        <div style={{fontWeight:700, color:'var(--color-primary)'}}>{product.price} {product.currency}</div>
        <div style={{display:'flex', gap:8}}>
          <Link to={`/product/${product.id}`} onClick={()=>dispatch(selectProduct(product))}><Button>View Details</Button></Link>
...
export default function Home() {
  ...
  return (
    <div className="container">
      <h1 className="display" style={{textAlign:'center', marginBottom:'1rem'}}>Greene Heaven</h1>
